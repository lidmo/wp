...
class Connection extends MySqlConnection
{
    ...
    public function select($query, $bindings = [], $useReadPdo = true)
    {
        return $this->run($query, $bindings, function ($query, $bindings) use ($useReadPdo) {
            ...
            $error=$this->WP->suppress_errors();
            $result = $this->WP->get_results($query);
            $this->WP->suppress_errors($error);
            if ($this->WP->last_error)
                throw new QueryException($query, $bindings, new Exception($this->WP->last_error));
            return $result;
        });
    ...
    protected function runRawQuery($query){
        return $this->run($query, [], function ($query) {
            ...
            $error=$this->WP->suppress_errors();
            $result = $this->WP->query($query);
            $this->WP->suppress_errors($error);
            if($this->WP->last_error){
                throw new QueryException($query, [], new Exception($this->WP->last_error));
            }
